/**
 * Example data for development and testing reference.
 * Use scripts/seedData.js to populate Firestore with this data.
 */

export const exampleUsers = [
  {
    id: "admin-user-id",
    name: "Admin User",
    email: "admin@example.com",
    role: "admin",
    approved: true,
    hostApproved: true,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "admin-readonly-user-id",
    name: "Readonly Admin",
    email: "readonly-admin@example.com",
    role: "admin_readonly",
    approved: true,
    hostApproved: true,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "host-user-id",
    name: "Host User",
    email: "host@example.com",
    role: "host",
    approved: true,
    hostApproved: true,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "regular-user-id",
    name: "John Doe",
    email: "user@example.com",
    role: "user",
    approved: true,
    hostApproved: true,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "regular-user-2-id",
    name: "Amira Hassan",
    email: "amira@example.com",
    role: "user",
    approved: true,
    hostApproved: true,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "pending-host-approved-id",
    name: "Pending Host",
    email: "pending-host@example.com",
    role: "user",
    approved: false,
    hostApproved: true,
    adminApproved: false,
    disabled: false,
  },
  {
    id: "pending-admin-approved-id",
    name: "Pending Admin",
    email: "pending-admin@example.com",
    role: "user",
    approved: false,
    hostApproved: false,
    adminApproved: true,
    disabled: false,
  },
  {
    id: "pending-user-id",
    name: "Jane Smith",
    email: "pending@example.com",
    role: "user",
    approved: false,
    hostApproved: false,
    adminApproved: false,
    disabled: false,
  },
];

export const exampleEvents = [
  {
    title: "Sunday Prayer Meeting",
    area: "North Area",
    church: "Grace Community Church",
    activityCode: "Egt625",
    activityName: "3",
    details: "Weekly community prayer session for all members.",
    startDate: "2026-06-15",
    startTime: "09:00",
    endTime: "10:30",
    audienceType: "everyone",
    audienceUserIds: [],
    creatorId: "regular-user-id",
    creatorName: "John Doe",
    createdById: "regular-user-id",
    createdByName: "John Doe",
    createdByRole: "user",
    supervisorId: "host-user-id",
    supervisorName: "Host User",
  },
  {
    title: "Youth Bible Study",
    area: "South Area",
    church: "Hope Fellowship",
    activityCode: "Egy549",
    activityName: "7",
    details: "Bible study session for youth members ages 13-18.",
    startDate: "2026-06-18",
    startTime: "18:00",
    endTime: "19:30",
    audienceType: "selected",
    audienceUserIds: ["regular-user-id", "regular-user-2-id"],
    creatorId: "regular-user-id",
    creatorName: "John Doe",
    createdById: "regular-user-id",
    createdByName: "John Doe",
    createdByRole: "user",
    supervisorId: "",
    supervisorName: "",
  },
  {
    title: "Community Visit",
    area: "East Area",
    church: "New Life Assembly",
    activityCode: "Egt553",
    activityName: "12",
    details: "Visit to new community members in the east district.",
    startDate: "2026-06-20",
    startTime: "14:00",
    endTime: "16:00",
    audienceType: "only-me",
    audienceUserIds: ["host-user-id"],
    creatorId: "host-user-id",
    creatorName: "Host User",
    createdById: "host-user-id",
    createdByName: "Host User",
    createdByRole: "host",
    supervisorId: "admin-user-id",
    supervisorName: "Admin User",
  },
  {
    title: "Admin Planning Session",
    area: "Central Area",
    church: "City Church",
    activityCode: "Adp101",
    activityName: "5",
    details: "Planning session led by the admin team.",
    startDate: "2026-06-22",
    startTime: "11:00",
    endTime: "12:30",
    audienceType: "everyone",
    audienceUserIds: [],
    creatorId: "admin-readonly-user-id",
    creatorName: "Readonly Admin",
    createdById: "admin-readonly-user-id",
    createdByName: "Readonly Admin",
    createdByRole: "admin_readonly",
    supervisorId: "admin-user-id",
    supervisorName: "Admin User",
  },
];

export const exampleComments = [
  {
    eventId: "event-1",
    userId: "host-user-id",
    userName: "Host User",
    text: "Great initiative! Please prepare the attendance sheet.",
  },
  {
    eventId: "event-1",
    userId: "admin-user-id",
    userName: "Admin User",
    text: "Approved. Make sure to report KPI metrics after the event.",
  },
  {
    eventId: "event-2",
    userId: "regular-user-2-id",
    userName: "Amira Hassan",
    text: "I will invite the youth team and follow up with the plan.",
  },
];

export const exampleNotifications = [
  {
    userId: "regular-user-id",
    title: "Account Approved",
    message: "Welcome John Doe! Your account has been approved.",
    read: true,
  },
  {
    userId: "regular-user-id",
    title: "New Comment",
    message: 'Host User commented on "Sunday Prayer Meeting"',
    read: false,
  },
  {
    userId: "host-user-id",
    title: "New Event",
    message: 'John Doe created a new event: "Youth Bible Study"',
    read: false,
  },
  {
    userId: "pending-admin-approved-id",
    title: "Pending Approval",
    message: "Your account is waiting for host approval.",
    read: false,
  },
];
