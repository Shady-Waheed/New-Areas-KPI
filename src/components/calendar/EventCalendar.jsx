...
import { buildOfficialHolidayCalendarEventsForRange, getOfficialHolidayLabelMapForRange, toLocalDateString } from '../../utils/officialHolidays'
...
function getEventErrorMessage(error) {
  ...
  if (code === 'failed-precondition') return 'Firestore index required — check Firebase Console'
...
export default function EventCalendar({ events }) {
  ...
  const calendarEvents = useMemo(() => {
    const workEvents = buildCalendarEvents(events, user, currentView, toISODateTime)
    const holidayEvents = buildOfficialHolidayCalendarEventsForRange(visibleRange.start, visibleRange.end)
  ...
  const handleEventClick = (info) => {
    ...
    const { eventData, isDaySummary, dayEvents, date } = info.event.extendedProps
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-3 sm:p-4 dark:border-gray-700 dark:bg-gray-900">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Calendar</h2>
        <Button size="sm" onClick={() => { setPrefillDate(null); setCreateOpen(true) }} className="w-full sm:w-auto">
          <Plus size={16} />
          New Event
        </Button>
      </div>
      ...
      <FullCalendar
        ...
        editable={false}
        selectable
        dayMaxEvents={isPrivileged && currentView === 'dayGridMonth' ? false : 3}
        moreLinkClick="popover"
      />

      <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
        المناسبات الرسمية تُحسب تلقائياً لأي سنة وتظهر بشكل خفيف ولا تؤثر على أحداث العمل.
      </p>
      ...
      >
        <EventForm
          ...
          onCancel={() => { setCreateOpen(false); setPrefillDate(null) }}
          loading={loading}
        />
      </Modal>
