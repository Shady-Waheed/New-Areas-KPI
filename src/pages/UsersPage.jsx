...
import { subscribeToUsers, approveUser, changeUserRole, toggleUserDisabled } from '../services/userService'
import { ROLES, ROLE_LABELS } from '../utils/constants'
import { formatTimestamp } from '../utils/formatters'
import { useAuth } from '../hooks/useAuth'
import toast from 'react-hot-toast'

function UserActions({ user, currentUser, actionLoading, onApprove, onToggleDisabled }) {
  return (
...
export default function UsersPage() {
  ...
  const handleApprove = async (user) => {
    ...
    try {
      await approveUser(user.id, user.name)
  ...
  return (
    <div className="page-stack">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-lg font-semibold text-gray-900 sm:text-xl dark:text-gray-100">User Management</h2>
        <SearchBar
          value={search}
          onChange={setSearch}
      ...
      <div className="space-y-3 md:hidden">
        ...
        {filteredUsers.length === 0 && (
          <div className="py-12 text-center text-sm text-gray-500 dark:text-gray-400">No users found</div>
        )}
      </div>
        ...
        <div className="table-responsive">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
              <tr>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Name</th>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Email</th>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Role</th>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Status</th>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Joined</th>
                <th className="px-4 py-3 font-medium text-gray-600 lg:px-6 dark:text-gray-400">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="px-4 py-4 text-gray-600 lg:px-6 dark:text-gray-400">{user.email}</td>
