...
export default function WaitingApprovalPage() {
  ...
  const adminApproved = user?.adminApproved === true;
  const hostApproved = user?.hostApproved === true;
  ...
  return (
    <Card className="auth-card-wrap text-center">
      ...
      <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
        مرحباً {user?.name}، حسابك قيد المراجعة من قبل الادمن و الـ Host. سيتم
        تحويلك تلقائياً للوحة التحكم فور الموافقة من الطرفين.
      </p>
      <div className="mt-4 rounded-lg bg-gray-50 px-4 py-3 text-left text-sm dark:bg-gray-800">
        ...
        <p>
          <span className="font-medium">Host Approval:</span>{" "}
          {hostApproved ? "Approved" : "Pending"}
        </p>
      </div>
